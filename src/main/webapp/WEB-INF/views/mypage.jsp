			...
			<div class="col-lg-8">
				<table id="info_table">
					<tr>
						<td>
							<div class="card">
								<span class="card-header">Your Profiles</span>
								<div class="card-body">
									<div class="list_area">
										<ul>
											<li>ID : ${loginMember.id }</li>
											<li>name : ${loginMember.name }</li>
											<li>username : ${loginMember.username }</li>
											<li>phone : ${loginMember.phone }</li>
											<li>email : ${loginMember.email }</li>
											<li>state : ${loginMember.statename }</li>
										</ul>
									</div>
									<div class="button_area">
										<button type="button" class="btn btn-success" id="toModifyBtn">modify</button>
									</div>
								</div>
							</div>
						</td>
						<td>
							<div class="card">
								<span class="card-header">Your Activities</span>
								<div class="card-body">
									<div class="list_area">
										<ul>
											<li>Registerd Date : ${fn:substring(loginMember.regdate, 0, 10) }</li>
											<li>Last Login : ${fn:substring(loginMember.lastlogin, 0, 10) }</li>
											<li>Post Written : ${loginMember.postCount }</li>
											<li>Comment Written : ${loginMember.replyCount }</li>
										</ul>
									</div>
								</div>
							</div>
					...
					<tr>
						<td colspan="2">
							<c:if test="${loginMember.state == 9 }">
								<button type="button" id="toAdmin" class="btn btn-secondary"
									onclick="toAdmin();">admin</button>
							</c:if>
							<c:if test="${loginMember.state != 9 }">
